...
public class EvilPizza extends Pizza {
    private final Topping[] EVIL_TOPPINGS = {Topping.SARDINES, Topping.PINEAPPLE};

    /** Constructor */
    public EvilPizza(Size size, boolean extraSauce, boolean extraCheese) {
        super(size, Sauce.TOMATO, extraSauce, extraCheese);
        Collections.addAll(toppings, EVIL_TOPPINGS);
    }
    ...
    public String toString() {
        ...
        return "[Evil] " + super.toString() + "$" + priceFormat.format(price());
    }

    /** Overridden price() method */
    @Override
    public double price() {
        double basePrice = 29.99; // base price for small size
